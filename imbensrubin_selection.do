...
global				VL patents
global				TEMP cited collaborators co_author funding0 publications ///
					trials
capture drop		inward_score bloc comsup
scalar				CL=invchi2(1,.90)	/* I&R (2015) set CL to 1; give same ///
										   result */
local				I=1
while				`I'<=6 {	/* (*) */
 scalar				CHI2=0
...
/* (*)	Optimise plus tard. Normalement, je n'ai pas besoin de parcourir les 6 variables
		de TEMP. Une fois VL augmentée de la variable ayant le plus grand LR, puis
		augmentée de la variable ayant le deuxième plus grand LR, ..., l'algorithme
		ne mettra plus la liste à jour une fois arrivée à la dernière variable passant le
		test. C'est cette variable qui sera sélectionnée tout le temps, car toutes les
		autres qui restent ne passent pas le test */
